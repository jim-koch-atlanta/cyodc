...
combat rounds narrate on Haiku while the engine owns every outcome. Monsters are
hand-seeded into rooms. Level progression + worldgen arrive in M4+.
...
and combat engine — so the game (including fights) is fully playable offline.
`GET /health` reports `{"llm_mode": "stub"}`. Set `ANTHROPIC_API_KEY` in `.env`
for the real Sonnet/Haiku-narrated game (`llm_mode: "anthropic"`).
...
| GET | `/api/session/{id}/state` | world state: HP/gold, position, room, exits, inventory, monsters here, active combat, and a fog-of-war ASCII map |
